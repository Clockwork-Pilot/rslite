...
**Command:** `cd $PROJECT_ROOT && git checkout -- src/ && python /unsafe_rust_fixer/unsafe-rust-fixer.py --match-patterns='*' --fix src/ && ./build_all.sh && python /unsafe_rust_fixer/unsafe-rust-fixer.py --match-patterns='*' --fix --dry-run src/   | tee /tmp/dry_run_idempotent_check.txt; grep -q 'Occurrences matched: 0' /tmp/dry_run_idempotent_check.txt   || { echo 'FAIL: expected Occurrences matched: 0 after fix'; exit 1; }; grep -q 'Fixes skipped (dry-run): 0' /tmp/dry_run_idempotent_check.txt   || { echo 'FAIL: expected Fixes skipped (dry-run): 0 after fix'; exit 1; }`
