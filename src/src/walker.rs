...
extern "C" {
    ...
    pub type RefSrcList;
    pub type CCurHint;
    fn sqlite3SelectPopWith(_: *mut Walker, _: *mut Select);
}
#[derive(Copy, Clone)]
#[repr(C)]
pub struct sqlite3 {
    ...
    if !(*p).pWinDefn.is_null() {
        ...
        if (*pWalker).xSelectCallback2
            ...
            || {
                ...
                !pParse.is_null() && (*pParse).eParseMode as ::core::ffi::c_int >= PARSE_MODE_RENAME
            }
            || (*pWalker).xSelectCallback2
                == Some(
                    sqlite3SelectPopWith as unsafe extern "C" fn(*mut Walker, *mut Select) -> (),
                )
        {
