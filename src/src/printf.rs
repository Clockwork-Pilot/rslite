...
pub unsafe fn sqlite3_str_vappendf_sqlfunc(
    ...
    let mut src = DirectArgs::new(pArgList);
    sqlite3_str_vappendf2(pAccum, fmt_start, &mut src);
}
...
pub unsafe fn sqlite3_str_vappendf_args(
    ...
    let mut src = ExtractedArgs::new(args);
    sqlite3_str_vappendf2(pAccum, fmt_start, &mut src);
}

unsafe fn sqlite3_str_vappendf_impl(
    pAccum: *mut crate::src::headers::sqliteInt_h::sqlite3_str,
    fmt_start: *const ::core::ffi::c_char,
    mut cursor: PrintfArgCursor,
) {
    ...
    loop {
        ...
        // Arguments are consumed lazily by each specifier via cursor methods.
        // This is critical for SQLFUNC direct mode where sqlite3_value_text()
        // may invalidate pointers from prior calls.
        ...
        match xtype {
            etPERCENT => {
                // Consume the None arg to keep cursor aligned with extract_printf_args
                if !cursor.is_direct() {
                    let _ = cursor.next_arg();
                }
            ...
            etCHARX => {
                if cursor.is_direct() {
                    // SQLFUNC single-pass: get text pointer fresh from PrintfArguments
                    let text_ptr = cursor.get_charx_text();
                    length = 1;
                    if !text_ptr.is_null() {
                        buf[0] = *text_ptr;
                    ...
                    // Pre-extracted mode: arg could be Str (SQLFUNC) or Char/Int (VaList)
                ...
                // Precision > 1: repeat the character (doubling strategy from C code)
                if precision > 1 {
                    ...
                    while precision > 0 {
                        ...
                        sqlite3_str_append(
                            pAccum,
                            (*pAccum).zText.offset(((*pAccum).nChar as i64 - nCopyBytes) as isize),
                            nCopyBytes as ::core::ffi::c_int,
                        );
                        precision = (precision as i64 - nPrior) as ::core::ffi::c_int;
                        nPrior *= 2;
                    }
                    if flag_leftjustify && width > 1 {
                        sqlite3_str_appendchar(pAccum, width - 1, ' ' as ::core::ffi::c_char);
                    }
            ...
            etSTRING | etDYNSTRING => {
                let raw_ptr = if cursor.is_direct() {
                    cursor.get_str()
                } else {
                ...
                bufpt = if raw_ptr.is_null() {
                    b"\0".as_ptr() as *mut ::core::ffi::c_char
                } else {
                    raw_ptr
                };
                ...
                if xtype == etDYNSTRING {
                    zExtra = raw_ptr; // Will be freed below; raw_ptr is null-safe (won't free static)
                }
            }
            ...
            etESCAPE_q | etESCAPE_Q | etESCAPE_w => {
                ...
                let mut escarg: *mut ::core::ffi::c_char = if cursor.is_direct() {
                    cursor.get_str()
                } else {
                    match cursor.next_arg() {
                        PrintfArg::Str(p) => p,
            ...
            etSIZE => {
                // %n — write current length; no arg consumed in SQLFUNC direct mode
                if !cursor.is_direct() {
                    if let PrintfArg::NOut(p) = cursor.next_arg() {
            ...
            etTOKEN => {
                ...
                // Never used in SQLFUNC direct mode
                if (*pAccum).printfFlags as ::core::ffi::c_int & crate::src::headers::sqliteInt_h::SQLITE_PRINTF_INTERNAL == 0 {
                    return;
                }
                let arg_tok = cursor.next_arg().clone();
                if flag_alternateform {
                    if let PrintfArg::Expr(pExpr) = arg_tok {
                        if !pExpr.is_null()
                    ...
                    if let PrintfArg::Token(pToken) = arg_tok {
            ...
            etSRCITEM => {
                ...
                let arg_src = cursor.next_arg().clone();
                if let PrintfArg::SrcItem(pItem) = arg_src {
                    if !pItem.is_null() {
            ...
            etORDINAL | etRADIX | etDECIMAL | etPOINTER => {
                ...
                if xtype == etORDINAL || xtype == etRADIX {
                    cThousand = 0;
                }
                ...
                if cursor.is_direct() {
                    // Direct mode: get int from PrintfArguments
                    let v = cursor.get_int();
                    if signed {
                        if v < 0 {
                ...
                // Zero-pad: inflate precision so zeros fill to width (accounting for prefix)
                if flag_zeropad && !flag_leftjustify
                    && precision < width - (if prefix != 0 { 1 } else { 0 })
                {
                    precision = width - (if prefix != 0 { 1 } else { 0 });
                }

                // Allocate output buffer — use buf for small values, dynamic for large
                let zOut: *mut ::core::ffi::c_char;
                if precision < etBUFSIZE - 10 - etBUFSIZE / 3 {
                    nOut = etBUFSIZE as ::core::ffi::c_int;
                    zOut = &raw mut buf as *mut ::core::ffi::c_char;
                } else {
                    let mut n = precision as u64 + 10;
                    if cThousand != 0 { n += precision as u64 / 3; }
                    zExtra = printfTempBuf(pAccum, n as crate::src::headers::sqlite3_h::sqlite3_int64);
                    zOut = zExtra;
                    if zOut.is_null() { return; }
                    nOut = n as ::core::ffi::c_int;
                }
                bufpt = zOut.offset(nOut as isize - 1);

                // Ordinal suffix for %r — write BEFORE digits (right-to-left)
                if xtype == etORDINAL {
                    static Z_ORD: &[u8; 8] = b"thstndrd";
                    let mut x = (longvalue % 10) as usize;
                    if x >= 4 || (longvalue / 10) % 10 == 1 {
                        x = 0;
                    }
                    bufpt = bufpt.offset(-1);
                    *bufpt.offset(1) = Z_ORD[x * 2 + 1] as ::core::ffi::c_char;
                    bufpt = bufpt.offset(-1);
                    *bufpt.offset(1) = Z_ORD[x * 2] as ::core::ffi::c_char;
                }

                // Convert digits (right to left) — NO comma insertion here
                {
                    let cset_ptr = &aDigits[cset..];
                    let base_u64 = base as u64;
                    loop {
                        bufpt = bufpt.offset(-1);
                        *bufpt.offset(1) = cset_ptr[(longvalue % base_u64) as usize];
                        longvalue /= base_u64;
                        if longvalue == 0 { break; }
                    }
                }
                bufpt = bufpt.offset(1);  // bufpt now points to first digit
                // length = distance from bufpt to zOut[nOut-1] inclusive
                length = (zOut.offset(nOut as isize - 1) as usize - bufpt as usize) as ::core::ffi::c_int + 1;
                ...
                // Thousands separator: insert commas by shifting digits
                if cThousand != 0 {
                    let nn_total = (length - 1) / 3; // Number of "," to insert
                    let mut nn = nn_total;
                    let mut ix = (length - 1) % 3 + 1;
                    bufpt = bufpt.offset(-(nn as isize));
                    let mut idx_c: ::core::ffi::c_int = 0;
                    while nn > 0 {
                        *bufpt.offset(idx_c as isize) = *bufpt.offset(idx_c as isize + nn as isize);
                        ix -= 1;
                        if ix == 0 {
                            idx_c += 1;
                            *bufpt.offset(idx_c as isize) = cThousand as ::core::ffi::c_char;
                            nn -= 1;
                            ix = 3;
                        }
                        idx_c += 1;
                ...
                // Prepend alternate form prefix ("0" or "0x")
                ...
                // Zero-pad was handled via precision inflation; clear so generic path uses spaces
                flag_zeropad = false;
                // bufpt and length are set; fall through to generic width-padding
            }
            ...
            etFLOAT | etEXP | etGENERIC => {
                ...
                let realvalue: ::core::ffi::c_double = if cursor.is_direct() {
                    cursor.get_double()
                } else {
                    match cursor.next_arg() {
...
pub trait PrintfArgs {
    unsafe fn get_int(&mut self) -> crate::src::headers::sqlite3_h::sqlite3_int64;
    unsafe fn get_uint(&mut self) -> crate::src::headers::sqlite3_h::sqlite_uint64;
...
impl PrintfArgs for DirectArgs {
    ...
    unsafe fn get_uint(&mut self) -> crate::src::headers::sqlite3_h::sqlite_uint64 {
        getIntArg(self.pArgList) as crate::src::headers::sqlite3_h::sqlite_uint64
    }
...
// ─── PrintfArgCursor: backward-compat wrapper using the trait ────────────
// Used by sqlite3_str_vappendf_impl and the _sqlfunc/_args entry points.

pub struct PrintfArgCursor<'a> {
    inner: &'a mut dyn PrintfArgs,
}

impl<'a> PrintfArgCursor<'a> {
    pub fn new_extracted(args: &'a mut ExtractedArgs<'a>) -> Self {
        Self { inner: args }
    }

    pub fn new_direct(args: &'a mut DirectArgs) -> Self {
        Self { inner: args }
    }
    ...
    pub fn next_arg(&mut self) -> PrintfArg {
        self.inner.next_arg()
    }
    ...
    pub unsafe fn get_charx_text(&mut self) -> *mut ::core::ffi::c_char {
        self.inner.get_charx_text()
    }
...
pub unsafe fn sqlite3_str_vappendf2(
    mut pAccum: *mut crate::src::headers::sqliteInt_h::sqlite3_str,
    mut fmt: *const ::core::ffi::c_char,
    cursor: &mut dyn PrintfArgs,
) {
    ...
    let mut prefix: ::core::ffi::c_char = 0;
    let mut longvalue: crate::src::headers::sqlite3_h::sqlite_uint64 = 0;
            ...
            match xtype as ::core::ffi::c_int {
                ...
                etCHARX => {
                    if bArgList {
                        ...
                        let mut ch: ::core::ffi::c_uint = cursor.get_char();
                        length = crate::src::src::utf::sqlite3AppendOneUtf8Character(
                            &raw mut buf as *mut ::core::ffi::c_char,
            ...
            match current_block {
                9031895888649199432 => {
                    ...
                    if __infop_ref.flags as ::core::ffi::c_int & FLAG_SIGNED != 0 {
                        let mut v: crate::src::ext::rtree::rtree::i64_0 = cursor.get_int();
                        if v < 0 as crate::src::ext::rtree::rtree::i64_0 {
                            longvalue = !v as crate::src::headers::sqlite3_h::sqlite_uint64;
                    ...
                    if xtype as ::core::ffi::c_int == etORDINAL {
                        ...
                        if x >= 4 as ::core::ffi::c_int
                            || longvalue
                                ...
                                == 1 as crate::src::headers::sqlite3_h::sqlite_uint64
                        {
